...
Example script:
python -i example_by_trans_shuffling.py -s human -olf list/golub/xaa -sn 1 -o1 output/golub/xaa.example.fa -o2 output/golub/xaa.example.pep_len
...
from simulate_by_trans_shuffling import example_shu
import subprocess
import argparse
import random
import os
    ...
    with open(output1, "w") as w1, open(output2, "w") as w2:
        for orf_id in get_elements(orf_list_file):
            ...
            if orf_type != "noncoding":
                ...
                lst_sim_all_orf_cds, lst_sim_main_orf_cds, lst_sim_uorf_cds, lst_sim_overlapping_uorf_cds, lst_sim_all_orf_pep_len, lst_sim_main_orf_pep_len, lst_sim_uorf_pep_len, lst_sim_overlapping_uorf_pep_len = example_shu(species, orf_id, transcript_seq, canonical_start, canonical_end, simulation_num)
            else:
