...
#s=mouse
#s=zebrafish
python add_info_to_minimal_orf_list_concise_merged.py -s $s -olf list/$s.intFDRs.sim2000.latest.orf_id -iff output/$s.intFDRs.sim2000.latest.txt -sff output/$s.transFDRs.sim2000.latest.txt -o output/$s.merged_FDR.0.txt

python add_info_to_minimal_orf_list_concise_merged.py -s $s -olf list/$s.orf_id.updated -iff output/$s.intFDRs.sim2000.updated.txt -sff output/$s.transFDRs.sim2000.updated.txt -o output/$s.merged_FDR.1.txt
...
if __name__ == "__main__":
        ...
        for line in r.readlines():
            if not line.startswith("#"):
                ...
                if feature == "exon":
                    ...
                    if gene_id.startswith("anti"):
                        gene_id_to_gene_name[gene_id] = gene_id
                        gene_id_to_gene_classification[gene_id] = ['non_coding', 'lncRNA']
 
                    if gene_id.startswith("anti"):
                        gene_id_to_gene_name[gene_id] = gene_id
                        gene_id_to_gene_classification[gene_id] = ['non_coding', 'lncRNA']

    with open("annotation/" + species + ".annotation.added.gtf", "r") as r:
        for line in r.readlines():
            if not line.startswith("#"):
                chrom, source, feature, start, end, score, strand, frame, attribute = line.rstrip().split("\t")
                if feature == "exon":
                    gene_id = fetch_id_in_gtf_attribute(attribute, "gene_id")
                    if gene_id.startswith("anti") or gene_id.startswith("CUT") or gene_id.startswith("SUT") or gene_id.startswith("Unit"):
                        transcript_id = fetch_id_in_gtf_attribute(attribute, "transcript_id")
                        transcript_id_to_gene_id[transcript_id] = gene_id
                        gene_id_to_gene_name[gene_id] = gene_id
                        gene_id_to_gene_classification[gene_id] = ['non_coding', 'lncRNA']
    ...
    with open(output, "w") as w:
        w.write("\t".join(["orf_id", "chrom", "strand", "orf_start", "orf_end", "orf_type", "orf_classification", "orf_start_codon", "orf_pep_len", "int_FDR_L_all", "shu_FDR_L_all", "max_FDR_L_all", "merged_FDR_L_all", "int_L", "int_T", "shu_L", "shu_T", "transcript_id", "transcript_len", "UTR5_len", "canonical_len", "UTR3_len", "gene_id", "gene_name", "gene_type", "gene_classification"]) + "\n") 
        for orf_id in orf_list:
            ...
            if float(orf_id_to_int_FDR_L_all[orf_id]) > 0:
                ...
                A += float(orf_id_to_int_FDR_L_all[orf_id])
            else:
                int_FDR_L_all = 1 / float(orf_id_to_int_FDR_L_all_T[orf_id])
                A += 1
            B += float(orf_id_to_int_FDR_L_all_T[orf_id])
            ...
            lst_to_print = [orf_id, chrom, strand, orf_start, orf_end, orf.orf_type, orf_classification, orf.start_codon, orf.pep_len, int_FDR_L_all, shu_FDR_L_all, max_FDR_L_all, merged_FDR_L_all, orf_id_to_int_FDR_L_all[orf_id], orf_id_to_int_FDR_L_all_T[orf_id], orf_id_to_shu_FDR_L_all[orf_id], orf_id_to_shu_FDR_L_all_T[orf_id], transcript_id, orf.transcript_len, UTR5_len, CDS_len, UTR3_len, gene_id, gene_name, gene_type, gene_classification]
