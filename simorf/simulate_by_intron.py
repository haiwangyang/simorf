...
def get_intron_simulation(exons, intron_ids, intron_fasta, dct_intron_GT_AG):
    if exons == []:
        pass
    else:
        whole_SIM_seq = ""
        for left, useq, right in exons: # for each exon
            ...
            if left == "AG" and right == "GT":
                ...
                while rGTloc == "" or rAGloc == "":
                    rid = random.choice(intron_ids) # random trans id
                    riseq = str(intron_fasta[rid])
                    riL = len(riseq)

                    GTlocs = dct_intron_GT_AG[rid]['GT']
                    AGlocs = dct_intron_GT_AG[rid]['AG']
            ...
            else: # non-splicing exon use random intron
                ranloc = ""
                while ranloc == "":
                    rid = random.choice(intron_ids)
                    riseq = str(intron_fasta[rid])
                    riL = len(riseq)
...
def main():
    ...
    dct_intron_GT_AG = pickle_to_object("/projects/b1080/hy/ribo/ribo/uORF_paper/annotation/" + species + ".dct_intron_GT_AG.pickle")
    ...
    for sim in range(simulation_num):
        ...
        print("intron simulated transcript:")
        simulated_transcript_seq = get_intron_simulation(exon_structure, intron_ids, intron_fasta, dct_intron_GT_AG)
